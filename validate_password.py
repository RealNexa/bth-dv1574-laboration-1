...
LARGE_LETTERS_AMOUNT = 1
SPECIAL_CHARACTERS = "!@*$"

def test_password_policy():
    """
    Function that checks to see if the passwordpolicy is valid.
    """
...
def contains(password, charset, minumum_count = 1):
    """
    Function that takes a password and looks if the password contains a specified ammount
    of the symbols that exists in the charset variable.
    """
    if minumum_count < 1:
        return True
...
def generate_pwd():
    ...
    amount_of_numbers = random.randint(
        NUMBER_AMOUNT,
        space_left-(SPECIAL_AMOUNT+SMALL_LETTERS_AMOUNT+LARGE_LETTERS_AMOUNT)
        )
    space_left -= amount_of_numbers

    amount_of_specials = random.randint(
        SPECIAL_AMOUNT,
        space_left-(SMALL_LETTERS_AMOUNT+LARGE_LETTERS_AMOUNT)
        )
    space_left -= amount_of_specials

    amount_of_small_letters = random.randint(
        SMALL_LETTERS_AMOUNT,
        space_left-(LARGE_LETTERS_AMOUNT)
        )
    space_left -= amount_of_small_letters

    amount_of_large_letters = random.randint(
        LARGE_LETTERS_AMOUNT,
        space_left
        )
...
def main():
    """Main function that asks user for a password"""
    valid_password = False

    while not valid_password:
        print("\n")
        password = input(
            "Input a password that you want to use\nor type 0 for a computer generated password: "
            )
